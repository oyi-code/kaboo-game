...
// ── Card with optional hover label ──
function Card({ card, faceUp, highlighted, onClick, small, disabled, peeking, anim, style: st, hoverLabel, onHover, onLeave }) {
...
// ── HandGrid with hover support ──
function HandGrid({ cards, small, faceUp, peek, hl, onClick, anim, hoveredIdx, onHoverIdx, onLeaveIdx, hoverLabelText }) {
...
export default function App() {
  ...
  const execAbil = async () => {
    ...
    if (ability === 'peekSelf' && selMy !== null) {
      ...
      setTimeout(() => {
        setSwapAnim(prev => prev ? { ...prev, phase: 'done' } : null);
      }, 2000);
      setTimeout(async () => {
      ...
      if (aStep === 1) {
        if (selMy !== null) {
          ...
          if (adv.status === 'roundEnd') setRevealed(true);
          swapPendingRef.current = adv;
          setTimeout(() => setSwapAnim(prev => prev ? { ...prev, phase: 'done' } : null), 2000);
          setTimeout(async () => {
            setSwapAnim(null); setPhase('start');
            if (swapPendingRef.current) { await save(swapPendingRef.current); swapPendingRef.current = null; }
            playSound('cardDeal');
      ...
      <div style={{ padding: 16, minHeight: '100vh' }}>
        ...
        {screen === 'game' && gameData && roomData && (
          <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 32px)', gap: 8 }}>
            ...
            {swapAnim && (
              <div className="overlay" style={{ background: 'rgba(0,0,0,.9)', zIndex: 70 }}>
                <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 36, color: S.gbright, textAlign: 'center', letterSpacing: 4 }}>
                  🔄 {lang === 'tr' ? 'KART DEĞİŞİMİ' : 'CARD SWAP'}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 40, marginTop: 24, position: 'relative', minHeight: 160 }}>
                  {/* Player 1 card */}
                  <div style={{
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
                    transform: swapAnim.phase === 'sliding' ? 'translateX(80px)' : 'translateX(0)',
                    transition: 'transform 1.5s cubic-bezier(0.4, 0, 0.2, 1)',
                  }}>
                    <div style={{ fontSize: 14, fontWeight: 700, color: S.gbright }}>{swapAnim.p1}</div>
                    <Card card={{ rank: '?', suit: '?' }} faceUp={false} />
                    <div style={{ fontSize: 12, color: S.dim }}>#{swapAnim.c1 + 1}</div>
                  </div>
                  {/* Arrow */}
                  <div style={{ fontSize: 32, color: S.gold, position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)' }}>⇄</div>
                  {/* Player 2 card */}
                  <div style={{
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8,
                    transform: swapAnim.phase === 'sliding' ? 'translateX(-80px)' : 'translateX(0)',
                    ...
                    <div style={{ fontSize: 14, fontWeight: 700, color: S.gbright }}>{swapAnim.p2}</div>
                    <Card card={{ rank: '?', suit: '?' }} faceUp={false} />
                    <div style={{ fontSize: 12, color: S.dim }}>#{swapAnim.c2 + 1}</div>
                  </div>
                </div>
                {swapAnim.phase === 'done' && (
                  <div style={{ marginTop: 20, padding: '10px 24px', background: 'rgba(212,168,67,.15)', border: `1px solid ${S.gold}`, borderRadius: 8, textAlign: 'center' }}>
                    <div style={{ fontSize: 16, color: S.gbright, fontWeight: 700 }}>
