...
export default function App() {
  ...
  // peeking → playing
  useEffect(() => {
    if (gameData?.status === 'peeking' && !iPeek && iPeekT === 0 && isHost) {
      const t2 = setTimeout(() => updateGameState(roomCode, { status: 'playing' }), 500);
      return () => clearTimeout(t2);
    }
  }, [gameData?.status, iPeek, iPeekT, isHost, roomCode]);
          ...
          <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 32px)', gap: 8 }}>
            ...
            <div style={{ display: 'flex', flexWrap: 'wrap', justifyContent: 'center', gap: 12 }}>
              {others.map(p => {
                ...
                const inSnapSelect = snapMode?.targetPid === p.id;
                return (
                  <div key={p.id} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 4, padding: '8px 10px', background: 'rgba(0,0,0,.15)', border: `2px solid ${active ? S.gbright : inSnapSelect ? '#f39c12' : 'rgba(255,255,255,.08)'}`, borderRadius: 12, minWidth: 100 }}>
                    ...
                    <HandGrid cards={hand} small faceUp={revealed}
                      onClick={ci => {
                        if (inSnapSelect) { executeSnap(p.id, ci); }
                        else if (ability && ['peekOther', 'blindSwap', 'lookSwap'].includes(ability) && aStep === 0) { setSelOp(p.id); setSelOc(ci); }
                      }}
                      hl={ci => (inSnapSelect) || (selOp === p.id && selOc === ci)} />
                    {/* TIK TIK button */}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: 16, background: 'rgba(0,0,0,.15)', borderTop: '1px solid rgba(212,168,67,.15)', borderRadius: '16px 16px 0 0', marginTop: 'auto' }}>
              ...
              <HandGrid cards={myHand} faceUp={revealed} peek={peekCards}
                hl={ci => (drawn && (phase === 'drawn' || phase === 'fromDiscard')) || (ability && ['peekSelf', 'blindSwap'].includes(ability) && selMy === ci) || (ability === 'lookSwap' && aStep === 1 && selMy === ci) || snapGiveMode || (snapMode?.targetPid === pid)}
                onClick={ci => {
                  if (snapGiveMode) { executeSnapGive(ci); }
                  else if (snapMode?.targetPid === pid) { executeSnap(pid, ci); }
              ...
              {/* Cancel snap mode */}
              {snapMode && <button className="btn bsm bs" onClick={() => setSnapMode(null)}>{t.cancel}</button>}
            </div>
