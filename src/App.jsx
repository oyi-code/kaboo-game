...
// ── Card with optional hover label + card number badge ──
function Card({ card, faceUp, highlighted, onClick, small, disabled, peeking, anim, style: st, hoverLabel, onHover, onLeave, cardNumber }) {
  ...
  return (
    <div className={`kc ${show ? 'fu' : 'fd'} ${highlighted ? 'hl' : ''} ${small ? 'sm' : ''} ${disabled ? 'dis' : ''} ${anim || ''}`}
      ...
      style={{ ...st, color: show ? (jk ? '#6b21a8' : red ? '#dc2626' : '#1e293b') : undefined, position: 'relative' }}>
      {show ? (
        <div className="kcf">
      ...
      {/* Card number badge */}
      {cardNumber != null && (
        <div style={{ position: 'absolute', bottom: small ? -8 : -10, left: '50%', transform: 'translateX(-50%)', background: 'rgba(0,0,0,.7)', color: '#d4a843', fontSize: small ? 9 : 11, fontFamily: "'JetBrains Mono',monospace", fontWeight: 700, padding: small ? '1px 4px' : '1px 6px', borderRadius: 4, border: '1px solid rgba(212,168,67,.3)', lineHeight: 1.3, zIndex: 5 }}>
          #{cardNumber}
        </div>
      )}
      {hoverLabel && (
        <div style={{ position: 'absolute', top: -28, left: '50%', transform: 'translateX(-50%)', background: 'rgba(243,156,18,.9)', color: '#fff', fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 4, whiteSpace: 'nowrap', pointerEvents: 'none', zIndex: 10 }}>
          ✓ {hoverLabel}
        </div>
      )}
    </div>
...
// ── HandGrid with hover support + card numbers ──
function HandGrid({ cards, small, faceUp, peek, hl, onClick, anim, hoveredIdx, onHoverIdx, onLeaveIdx, hoverLabelText, showNumbers }) {
  const base = cards.slice(0, 4), pen = cards.slice(4), rows = [];
  for (let i = 0; i < pen.length; i += 2) rows.push(pen.slice(i, i + 2));
  const g = small ? 3 : 6;
  const nums = showNumbers !== false; // default true
  const rc = (c, idx) => <Card key={idx} card={c} faceUp={faceUp} small={small} peeking={peek?.[idx]}
    ...
    cardNumber={nums ? idx + 1 : null}
    hoverLabel={hoveredIdx === idx ? hoverLabelText : null}
    onHover={() => onHoverIdx?.(idx)}
    onLeave={() => onLeaveIdx?.()} />;
  return (
    <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: small ? g + 8 : g + 10 }}>
...
export default function App() {
  ...
  const [swapAnim, setSwapAnim] = useState(null); // { player1, card1Idx, player2, card2Idx, phase: 'sliding'|'done' }
  const swapPendingRef = useRef(null); // stores the game state to save after animation
  ...
  // ── Peek — triggered for ALL players when status becomes 'peeking' ──
  ...
  useEffect(() => {
    if (gameData?.status === 'peeking' && gameData?.round > 0 && gameData.round !== lastPeekRoundRef.current) {
      ...
      setDrawn(null); setPhase('start'); setAbility(null); setRevealed(false); setTempCard(null); setSnapMode(null); setSnapGiveMode(false);
      playSound('cardDeal');
    }
  ...
  // peeking → playing (only host triggers the transition)
  ...
  // ── Swap animation — trigger for NON-host players from Firebase lastAction ──
  ...
  const execAbil = async () => {
    ...
    if (ability === 'peekSelf' && selMy !== null) {
      ...
      // Broadcast swap animation to other players
      ...
      // Write intermediate state so others see the animation action
      await save({ ...gs, currentPlayerIndex: adv.currentPlayerIndex });
      setTimeout(() => setSwapAnim(prev => prev ? { ...prev, phase: 'done' } : null), 4000);
      setTimeout(async () => {
        setSwapAnim(null); setPhase('start');
        if (swapPendingRef.current) { await save(swapPendingRef.current); swapPendingRef.current = null; }
        playSound('cardDeal');
      }, 5000);
      ...
      <div style={{ padding: 16, minHeight: '100vh' }}>
        ...
        {screen === 'game' && gameData && roomData && (
          <div style={{ display: 'flex', flexDirection: 'column', minHeight: 'calc(100vh - 32px)', gap: 8 }}>
            ...
            {/* SNAP SELECT OVERLAY — hover to select, NO cancel */}
            {snapMode && !snapGiveMode && (
              <div className="overlay" style={{ background: 'rgba(0,0,0,.85)', zIndex: 55 }}>
                ...
                <HandGrid
                  ...
                  hl={ci => hovIdx === ci}
                  hoveredIdx={hovIdx}
                  onHoverIdx={ci => setHovIdx(ci)}
                  onLeaveIdx={() => setHovIdx(null)}
                  hoverLabelText={lang === 'tr' ? 'Seçilen Kart' : 'Selected Card'}
            ...
            <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10, padding: 16, background: 'rgba(0,0,0,.15)', borderTop: '1px solid rgba(212,168,67,.15)', borderRadius: '16px 16px 0 0', marginTop: 'auto' }}>
              ...
              <HandGrid cards={myHand} faceUp={revealed} peek={peekCards}
                ...
                hoverLabelText={(drawn && (phase === 'drawn' || phase === 'fromDiscard')) ? (lang === 'tr' ? 'Kartı Buraya Koy' : 'Place Card Here') : undefined}
                onClick={ci => {
            ...
            {/* SWAP ANIMATION — shows when J/Q/K swap happens */}
            {swapAnim && (
              <div className="overlay" style={{ background: 'rgba(0,0,0,.92)', zIndex: 70 }}>
                <div style={{ fontFamily: "'Playfair Display',serif", fontSize: 40, color: S.gbright, textAlign: 'center', letterSpacing: 6, textShadow: '0 0 30px rgba(212,168,67,.4)' }}>
                  🔄 {lang === 'tr' ? 'KART DEĞİŞİMİ' : 'CARD SWAP'}
                </div>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 60, marginTop: 32, position: 'relative', minHeight: 200 }}>
                  ...
                  <div style={{
                    ...
                    transform: swapAnim.phase === 'sliding' ? 'translateX(100px)' : 'translateX(0)',
                  ...
                  {/* Center arrow */}
                  <div style={{ fontSize: 40, color: S.gold, position: 'absolute', left: '50%', top: '50%', transform: 'translate(-50%, -50%)', textShadow: '0 0 20px rgba(212,168,67,.5)', opacity: swapAnim.phase === 'sliding' ? 1 : 0.5, transition: 'opacity 1s' }}>⇄</div>
                  {/* Player 2 card */}
                  <div style={{
                    display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 10,
                    transform: swapAnim.phase === 'sliding' ? 'translateX(-100px)' : 'translateX(0)',
                ...
                {swapAnim.phase === 'done' && (
                  <div style={{ marginTop: 24, padding: '14px 32px', background: 'rgba(212,168,67,.12)', border: `2px solid ${S.gold}`, borderRadius: 12, textAlign: 'center', animation: 'din .4s ease-out' }}>
                    <div style={{ fontSize: 20, color: S.gbright, fontWeight: 700, letterSpacing: 2 }}>
                      {swapAnim.p1} #{swapAnim.c1 + 1} ⇄ {swapAnim.p2} #{swapAnim.c2 + 1}
                    </div>
                    <div style={{ fontSize: 15, color: S.dim, marginTop: 6 }}>
                      {lang === 'tr' ? 'Kartlar değiştirildi!' : 'Cards swapped!'}
                    </div>
                  </div>
                )}
              </div>
            ...
            {/* Ability modal — with HandGrid + hover labels */}
            {ability && !Object.keys(peekCards).length && !tempCard && <div className="overlay" style={{ background: 'rgba(0,0,0,.85)' }}>
              ...
              {/* Player select */}
              {['peekOther', 'blindSwap', 'lookSwap'].includes(ability) && !selOp && aStep === 0 && (
                <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', justifyContent: 'center' }}>
                  {others.map(p => <button key={p.id} className="btn bsm bs" onClick={() => setSelOp(p.id)}>{p.name}</button>)}
                </div>
              )}

              {/* Opponent cards — HandGrid with hover */}
              {selOp && (['peekOther', 'blindSwap'].includes(ability) || (ability === 'lookSwap' && aStep === 0)) && (
                <div>
                  <div style={{ color: S.light, fontSize: 14, fontWeight: 600, textAlign: 'center', marginBottom: 6 }}>
                    {players.find(p => p.id === selOp)?.name}
                  </div>
                  <HandGrid
                    cards={gameData.hands[selOp] || []}
                    faceUp={false}
                    hl={ci => selOc === ci || hovIdx === ci}
                    hoveredIdx={hovIdx}
                    onHoverIdx={ci => setHovIdx(ci)}
                    onLeaveIdx={() => setHovIdx(null)}
                    hoverLabelText={lang === 'tr' ? 'Seçilen Kart' : 'Selected Card'}
                    onClick={ci => { setSelOc(ci); setHovIdx(null); }}
                  />
                </div>
              )}
              ...
              {ability === 'peekSelf' && (
                <div>
                  <div style={{ color: S.light, fontSize: 14, fontWeight: 600, textAlign: 'center', marginBottom: 6 }}>{pname}</div>
                  <HandGrid
                    cards={myHand}
                    faceUp={false}
                    hl={ci => selMy === ci || hovIdx === ci}
                    hoveredIdx={hovIdx}
                    onHoverIdx={ci => setHovIdx(ci)}
                    onLeaveIdx={() => setHovIdx(null)}
                    hoverLabelText={lang === 'tr' ? 'Seçilen Kart' : 'Selected Card'}
                    onClick={ci => { setSelMy(ci); setHovIdx(null); }}
                  />
                </div>
              )}

              {/* Own cards — blindSwap (after opponent selected) */}
              {ability === 'blindSwap' && selOp && selOc !== null && (
                <div>
                  <div style={{ color: S.dim, marginBottom: 6, fontSize: 13, textAlign: 'center' }}>{t.selectYourCard}</div>
                  <HandGrid
                    cards={myHand}
                    faceUp={false}
                    hl={ci => selMy === ci || hovIdx === ci}
              ...
              {/* Own cards — lookSwap step 1 */}
              {ability === 'lookSwap' && aStep === 1 && (
                <div>
                  <div style={{ color: S.dim, marginBottom: 6, fontSize: 13, textAlign: 'center' }}>{t.selectYourCard}</div>
                  <HandGrid
                    cards={myHand}
