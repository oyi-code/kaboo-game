              ...
              <div className="overlay" style={{ background: 'rgba(0,0,0,.8)', zIndex: 55 }}>
                ...
                <div style={{ position: 'relative' }}>
                  <HandGrid
                    ...
                    faceUp={false}
                    hl={ci => snapMode.selectedCard === ci}
                    onClick={ci => setSnapMode(prev => ({ ...prev, selectedCard: ci }))}
                  />
                  {/* "Seçilen Kart" label */}
                  {snapMode.selectedCard != null && (() => {
                    const cards = gameData.hands?.[snapMode.targetPid] || [];
                    const total = cards.length;
                    const penRows = Math.ceil(Math.max(0, total - 4) / 2);
                    const ci = snapMode.selectedCard;
                    let row, col;
                    if (ci >= 4) { const pi = ci - 4; row = Math.floor(pi / 2); col = pi % 2; }
                    else if (ci < 2) { row = penRows; col = ci; }
                    else { row = penRows + 1; col = ci - 2; }
                    return null; // label handled below
                  })()}
                </div>
                {snapMode.selectedCard != null && (
                  <div style={{ marginTop: 8, padding: '6px 16px', background: 'rgba(243,156,18,.2)', border: '1px solid #f39c12', borderRadius: 8, color: '#f39c12', fontWeight: 700, fontSize: 14 }}>
                    ✓ {lang === 'tr' ? 'Seçilen Kart' : 'Selected Card'}: #{snapMode.selectedCard + 1}
                  </div>
                )}
                <div style={{ display: 'flex', gap: 8, marginTop: 8 }}>
                  {snapMode.selectedCard != null && (
                    <button className="btn bp bsm" onClick={() => executeSnap(snapMode.targetPid, snapMode.selectedCard)}>
                      {t.confirm} 👊
                    </button>
                  )}
                  <button className="btn bs bsm" onClick={() => setSnapMode(null)}>{t.cancel}</button>
                </div>
              </div>
            )}
